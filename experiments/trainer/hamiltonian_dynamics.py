...
class HamiltonianDataset(Dataset):
    ...
    def generate_trajectory_data(
        ...
        zs: (n_systems, traj_len, z_dim) q1,q2,p1,p2
        pv: (n_systems, 3) g 
        ps: (n_systems, 6) m1,m2,k1,k2,l1,l2 
        """
        if not rescaleKG:
    ...
    def H(self,z,pv=None,ps=None):
        """ The Hamiltonian function, depending on z=pack(q,p), pv=g, ps=m1,m2,k1,k2,l1,l2)"""
    ...
    def animate(self, zt=None, pvt=None, pst=None):
        ...
        if zt is None:
            ...
            zt = np.asarray(self.integrate(self.sample_initial_conditions(10)[0],self.T_long,pvt,pst))
...
class GeneralData(object):
    def __init__(
        ...
        test_id = np.random.choice(ndata, (ntest,), replace=False)  
        ...
        for i in range(len(testname_all)):
            ...
            metrics = np.exp(
                self.evalAverageMetrics(
                    loader, 
                    partial(log_rollout_error, loader.dataset, self.model)
                )
            )
...
class IntegratedODETrainer(Regressor):
    ...
    def logStuff(self, step, minibatch=None):
        ...
        metrics = {'test_Rollout': np.exp(
            self.evalAverageMetrics(
                loader,
                partial(log_rollout_error_ode,loader.dataset,self.model)
            )
        )} 
        self.logger.add_scalars('metrics', metrics, step)
        super().logStuff(step,minibatch)
...
class GeneralDynamicsTrainer(Regressor):
    ...
    def logStuff(self, step, minibatch=None):
        ...
        metrics = {'test_Rollout': np.exp(
            self.evalAverageMetrics(
                loader,
                partial(log_rollout_error_general,self.model)
            )
        )}
        self.logger.add_scalars('metrics', metrics, step)
        super().logStuff(step,minibatch)
